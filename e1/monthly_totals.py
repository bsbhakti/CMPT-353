...
def pivot_months_pandas(data):
    ...
    data["formattedDate"] = data["date"].apply(date_to_month)
    grouped_data = data.groupby(['formattedDate','name'])
    precipitationSum = (grouped_data.aggregate({'precipitation': 'sum'}).reset_index())
    totals = precipitationSum.pivot(index="name",columns="formattedDate",values="precipitation")
    countSum = (grouped_data.aggregate({'precipitation': 'count'}).reset_index())
    counts = countSum.pivot(index="name",columns="formattedDate",values="precipitation")
